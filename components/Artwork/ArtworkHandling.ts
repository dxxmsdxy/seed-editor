...
export const ArtworkHandling: React.FC<ArtworkHandlingProps> = ({
    ...
    const updateSVGClasses = useCallback(() => {
        ...
        if (svgRef.current) {
            ...
            if (rgblens) {
                if (modValues.tint === 0) {
                    ...
                    // Special treatment for tintPercent
                    if (modValues.tintPercent === 100) {
                        rgblens.style.opacity = '1';
                    } else {
                        rgblens.style.opacity = (modValues.tintPercent / 100).toString();
                    }
                }
            }
    ...
    }, [displaySettings, modValues, editorSeed, editorAttunement, isColorAnimationPaused, isDepthAnimationPaused, isSpinAnimationPaused]);
